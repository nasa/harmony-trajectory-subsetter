...
protected:
  void SetUp() {
    ...
    configFile =
        std::regex_replace(currentDir, std::regex("tests/unit/gtest/build"),
...
TEST_F(test_ATL10_v006_Configuration,
  ...
  groupname =
      configuration_a->getIndexBeginDatasetName(shortName, group, datasetName);
  EXPECT_EQ(expectGroupname, groupname);
}
...
TEST_F(
  ...
  groupname = configuration_a->getIndexBeginDatasetName(shortName, group,
                                                        datasetName, repair);
  EXPECT_EQ(expectGroupname, groupname);
}
...
TEST_F(test_ATL10_v006_Configuration, ATL10_isShortNameGrougInH5File) {
  ...
  bool bisShortNameGrougInH5File =
      configuration_a->isShortNameGroupDatasetFromGranuleFile(shortName, group);
