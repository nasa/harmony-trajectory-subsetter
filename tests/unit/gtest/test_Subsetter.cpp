...
public:
  ...
  Coordinate *getCoordinate(H5::Group &root, H5::Group &inputGroup,
    ...
    coor = std::make_unique<Coordinate>(groupName, geoboxes, temporal,
                                        geoPolygon, config);
...
protected:
  SubsetterTest() {
    std::string config_file_path =
        gtest_utilities::getFullPath("harmony_service/subsetter_config.json");
  ...
  void createCollectionSubsetter(
      std::string &shortName, H5::H5File &inputFileName,
      const std::vector<std::string> &groupsWithOnlyTemporalCoordinates) {
    subsetDataLayers = std::make_unique<SubsetDataLayers>(variables);

    subsetter = std::make_unique<StubSubsetter>(
        subsetDataLayers.get(), geoboxes.get(), temporal.get(),
        geopolygon.get(), config.get(), shortName, inputFileName,
        groupsWithOnlyTemporalCoordinates, variables);
  }
...
TEST_F(SubsetterTest,
  ...
  variables = {"/gt1l/bckgrd_atlas/bckgrd_counts", "/gt1l/geolocation/",
               "/gt1l/signal_find_output/land/bckgrd_mean"};

  createCollectionSubsetter(shortName, inputFileATL03,
                            atl03GroupsWithOnlyTemporalCoordinates);

  subsetter->addGroupsRequiringTemporalSubsetting(rootGroupATL03,
                                                  rootGroupATL03, "/");

  std::vector<std::string> actual_groups =
      subsetter->getGroupsRequiringTemporalSubsetting();

  std::vector<std::string> expected_groups = {"/gt1l/bckgrd_atlas/",
                                              "/gt1l/signal_find_output/land/"};
...
TEST_F(SubsetterTest,
  ...
  variables = {"/gt1l/geolocation/latitude",
               "/gt1l/geolocation/solar_elevation", "/gt1l/heights/h_ph"};
...
TEST_F(SubsetterTest,
  ...
  createCollectionSubsetter(shortName, inputFileATL10,
                            atl10GroupsWithOnlyTemporalCoordinates);

  subsetter->addGroupsRequiringTemporalSubsetting(rootGroupATL10,
                                                  rootGroupATL10, "/");
...
TEST_F(SubsetterTest,
  ...
  H5::H5File infilename = H5::H5File(
      gtest_utilities::getFullPath("tests/data/variable_subset_ATL24_data.h5"),
      H5F_ACC_RDONLY);
  ...
  createCollectionSubsetter(shortName, infilename, std::vector<std::string>());
...
TEST_F(SubsetterTest,
  ...
  H5::H5File infilename = H5::H5File(
      gtest_utilities::getFullPath("tests/data/variable_subset_ATL24_data.h5"),
      H5F_ACC_RDONLY);
  H5::H5File outfilename = H5::H5File(
      gtest_utilities::getFullPath("tests/data/variable_subset_ATL24_data.h5"),
      H5F_ACC_RDONLY);
