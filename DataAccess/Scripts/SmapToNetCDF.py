...
parameters = ParameterHandling("SMAP HDF5 Reformat to NetCDF-4/HDF5 Tool Adapter");
filepath = sys.path[0];
...
if inputfile.lower().endswith(".xml") and parameters.isMetadataIncluded():
    ...
    # remove ".h5" extension in the middle of the output filename
    ...
    cmd = scriptpath + " --filename " + inputfile + " --outfile " + outputfile; 
    # used to remove paths for readable logging
    internalCmd = os.path.basename(scriptpath) + " --filename " + os.path.basename(inputfile) + \
    ...
    if projection is not None:
        ...
        internalCmd += " --projection " + projection;
...
if (exitstatus == 0) and os.path.isfile(outputfile):
    ...
    errorMsg = "SmapToNetCDF failed with exit code"
