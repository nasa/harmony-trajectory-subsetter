...
public:
  ...
  SubsetDataLayers(std::vector<std::string> dataset_to_include)
    ...
    if (!dataset_to_include.empty()) {
      include_all = false;
      for (std::vector<std::string>::iterator it = dataset_to_include.begin();
           it != dataset_to_include.end(); it++) {
        std::string dataset;
        ...
        for (; iter != tokens.end(); iter++) {
          ...
          if (*dataset.rbegin() != '/')
            dataset.push_back('/');
          LOG_DEBUG(
              "SubsetDataLayers::SubsetDataLayers(): adding dataset to include "
              << dataset);
          add_dataset(dataset);
        }
      }
    ...
    // else, include all datasets
      ...
      LOG_DEBUG("SubsetDataLayers::SubsetDataLayers(): adding / dataset to "
                "include all");
      include_all = true;
      add_dataset("/");
    }
  ...
  void print_datasets() {
    LOG_DEBUG(
        "SubsetDataLayers::print_datasets(): printing the subset data layers");
  ...
  void write_to_json(std::string json_out_name) {
    ...
    LOG_DEBUG("SubsetDataLayers::write_to_json(): writing to the json file: "
              << json_out_name);
...
private:
  ...
  void add_dataset(std::string str) {
    ...
    // ancestors. Step down through levels of datasets and exit if ancestor is
    // found.
  ...
  void read_from_json(std::string json_in_name) {
    LOG_DEBUG("SubsetDataLayers::read_from_json(): reading from the json file: "
              << json_in_name);
