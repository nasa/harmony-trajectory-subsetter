...
public:
  HeightSegmentCoordinates(std::string groupname, std::vector<geobox> *geoboxes,
  ...
  static Coordinate *getCoordinate(H5::Group &root, H5::Group &ingroup,
    ...
    if (Coordinate::lookUp(groupname)) {
      LOG_DEBUG("HeightSegmentCoordinates::getCoordinate(): groupname: "
                << groupname
    ...
    if (Coordinate::lookUp(leadsGroupname))
      ...
      leadsCoor = ForwardReferenceCoordinates::getCoordinate(
          root, hgtSegCoor->leadsGroup, shortName, subsetDataLayers,
          leadsGroupname, geoboxes, temporal, geoPolygon, config);
  ...
  virtual IndexSelection *getIndexSelection() {
    ...
    if (!indexBegName.empty() &&
        H5Lexists(leadsGroup.getLocId(), indexBegName.c_str(), H5P_DEFAULT) > 0)
      indexBegSet = new H5::DataSet(leadsGroup.openDataSet(indexBegName));
    if (!countName.empty() &&
        H5Lexists(leadsGroup.getLocId(), countName.c_str(), H5P_DEFAULT) > 0)
    ...
    for (std::map<long, long>::iterator it = localIndexes->segments.begin();
         it != localIndexes->segments.end(); it++) {
    ...
    for (std::map<long, long>::iterator it = leadsIndexes->segments.begin();
         it != leadsIndexes->segments.end(); it++) {
      start = it->first;
      length = it->second;
