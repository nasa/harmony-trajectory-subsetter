...
public:
  ...
  static Coordinate *getCoordinate(H5::Group &root, H5::Group &ingroup,
    ...
    LOG_DEBUG("ReverseReferenceCoordinates::getCoordinate(): ENTER groupname:"
              << groupname);

    ReverseReferenceCoordinates *reverseCoor = new ReverseReferenceCoordinates(
    ...
    if (Coordinate::lookUp(referencedGroupname))
      referencedCoor = Coordinate::lookUpMap[referencedGroupname];
                                                  ...
                                                  referencedGroupname))
      referencedCoor = Coordinate::getCoordinate(
          root, referencedGroup, referencedGroupname, reverseCoor->shortname,
      ...
      referencedCoor = ReverseReferenceCoordinates::getCoordinate(
          root, referencedGroup, reverseCoor->shortname, subsetDataLayers,
          referencedGroupname, geoboxes, temporal, geoPolygon, config);
  ...
  virtual IndexSelection *getIndexSelection() {
    ...
    if (!indexBegName.empty() and
        (H5Lexists(ingroup.getLocId(), indexBegName.c_str(), H5P_DEFAULT) >
         0)) {
      indexBegSet =
          std::make_shared<H5::DataSet>(ingroup.openDataSet(indexBegName));
    }
...
private:
  ...
  void reverseSubset(H5::DataSet *indexBegSet) {
    ...
    LOG_DEBUG("ReverseReferenceCoordinates::reverseSubset(): coordinateSize:"
              << coordinateSize);
    indexBegSet->read(indexBegin, indexBegSet->getDataType());
    long start = 0, length = 0, end = 0, count = 0, newStart = 0, newLength = 0;

    // iterate through index selection of the referenced group (i.e., for leads
    // group ,iterate through freeboard swath group) if the value in the index
    // begin dataset  matches the indices in the index selection, calculate the
    // index range for that value, add it the index selection
    for (std::map<long, long>::iterator it =
             referencedIndexes->segments.begin();
         it != referencedIndexes->segments.end(); it++) {
      start = it->first + 1;
      length = it->second;
      end = start + length;
