...
public:
   ...
   */
  void writeDataset(H5::Group &outgroup, const std::string &groupname,
    ...
    // Writing the segment begin datasets requires copying the selected values
    // to the output, minus an offset.  The offset reflects the extent of
    // previous values in the target segmented datasets that have not been
    // written, thus shifting the forward reference begin index to an earlier
    // value.  E.g., we know the first segment will begin at the first position
    // in the output segment begin datatset
    // - there are no preceding data items in the subsetted target dataset. The
    // offset in this case is the source forward-reference-begin value for this
    // first selected segment. Subtracting the offset yields zero for the output
    // value, + 1 since index values start at one.
    ...
    // For each selected element group, write to the segment begin output/subset
    // dataset.
    ...
    for (std::map<long, long>::iterator it = allSegments.begin();
      ...
      for (int i = skippedFillValueCnt; i < segmentCountIn; i++) {
        if (subsetBeginIn[segmentBeginIn] <= 0) {
          subsetBeginOut[segmentBeginOut] = subsetBeginIn[segmentBeginIn];
          // 0 for ATL3, ATL8, -1 for ATL10
        } else {
