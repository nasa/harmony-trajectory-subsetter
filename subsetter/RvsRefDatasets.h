...
public:
   ...
   */
  void mapWriteDataset(H5::Group &outgroup, const std::string &groupname,
                       const H5::DataSet &indataset, IndexSelection *indexes,
                       IndexSelection *targetIndexes,
                       SubsetDataLayers *subsetDataLayers) {
    LOG_DEBUG(
        "RvsRefDatasets::mapWriteDataset(): ENTER groupname: " << groupname);
    ...
    int32_t *indexRef = new int32_t[subsettedSize]; // array to store subsetted
                                                    // index reference dataset
    int32_t *newIndexRef =
        new int32_t[subsettedSize]; // array to store repaired index reference
                                    // dataset
    ...
    // count - keeps track of how many indices in the (start, length) have been
    // accounted for prevOldLocation - previous index
    ...
    for (int i = 0; i < subsettedSize; i++) {
      ...
      if (!targetIndexes->segments.empty()) {
        for (std::map<long, long>::iterator it = startIter;
             it != targetIndexes->segments.end(); it++) {
      ...
      // first index
      // calculate offset if first index does not equal to start of the (start,
      // length) pair in targetIndexes
      ...
      // new index = previous new index + offset
        ...
        // diffIndex - index is in a different (start, length) pair than the
