...
protected:
  ...
  virtual void writeDataset(const std::string &objname,
    ...
    if (indexes != NULL && indexes->getMaxSize() != indexes->size() &&
        ...
        config->getIndexBeginDatasetName(this->getShortName(), groupname,
                                         objname) == objname) {
      // need to make sure count dataset exists
      std::string countName =
          config->getCountDatasetName(this->getShortName(), groupname, objname);
      // if the count dataset is not in the output file, create it
      if (H5Lexists(outgroup.getLocId(), countName.c_str(), H5P_DEFAULT) <= 0) {
        // if the input file doesn't have the count dataset, write index begin
        // as normal dataset
        if (H5Lexists(infile.getLocId(), (groupname + countName).c_str(),
                      H5P_DEFAULT) <= 0) {
          Subsetter::writeDataset(objname, indataset, outgroup, groupname,
                                  indexes);
          return;
        }
        ...
        Subsetter::writeDataset(countName, inCountDs, outgroup, groupname,
                                indexes);
      ...
      photonDataset->writeDataset(outgroup, groupname, indataset, indexes,
    ...
    // write index begin for ATL10 if it has been subsetted
      ...
      std::string targetGroupname =
          config->getTargetGroupname(this->getShortName(), groupname, objname);
      LOG_DEBUG("IcesatSubsetter::writeDataset() groupname: "
                << groupname << " objname: " << objname
                << " targetGroupname: " << targetGroupname);

      // if target group does not exist in input, write index begin as normal
      // dataset
      if (H5Lexists(infile.getLocId(), targetGroupname.c_str(), H5P_DEFAULT) <=
          0) {
        Subsetter::writeDataset(objname, indataset, outgroup, groupname,
                                indexes);
        return;
      ...
      if (Coordinate::lookUp(targetGroupname))
        ...
        coor = IcesatSubsetter::getCoordinate(
            root, targetGroup, targetGroupname, this->getSubsetDataLayers(),
            this->getGeobox(), this->getTemporal(), this->getGeoPolygon(),
            config, true);
      ...
      // write index begin
      referenceDataset->mapWriteDataset(outgroup, groupname, indataset, indexes,
                                        targetIndexes,
                                        this->getSubsetDataLayers());
...
private:
  virtual Coordinate *getCoordinate(H5::Group &root, H5::Group &ingroup,
    ...
    bool isFreeboardBeamSegmentGroup =
    ...
    if (hasPhotonSegmentGroup && (isPhotonGroup || isLeadsGroup) &&
      ...
      return ForwardReferenceCoordinates::getCoordinate(
          root, ingroup, this->getShortName(), subsetDataLayers, groupname,
      ...
      if (isHeightsGroup || isGeophysicalGroup) {
        coorGroupname =
            config->getBeamFreeboardGroup(this->getShortName(), groupname);
        coorGroup = root.openGroup(coorGroupname);
      } else if (isFreeboardSegmentHeightsGroup ||
                 isFreeboardSegmentGeophysicalGroup) {
        coorGroupname =
            config->getFreeboardSegmentGroup(this->getShortName(), groupname);
        coorGroup = root.openGroup(coorGroupname);
      }
      ...
      return ReverseReferenceCoordinates::getCoordinate(
          root, coorGroup, this->getShortName(), subsetDataLayers,
      ...
      LOG_DEBUG("IcesatSubsetter::getCoordinate(): Call "
                ...
                << groupname);
      return SuperGroupCoordinate::getCoordinate(
          root, ingroup, this->getShortName(), subsetDataLayers, groupname,
          geoboxes, temporal, geoPolygon, config);
    } else {
      LOG_DEBUG("IcesatSubsetter::getCoordinate(): Call "
                ...
                << groupname);
      return Subsetter::getCoordinate(root, ingroup, groupname,
  ...
  bool checkFreeboardSwathSegmentExists(H5::Group &root,
    ...
    if (freeboardSwathSegmentName == "" ||
        H5Lexists(root.getLocId(), freeboardSwathSegmentName.c_str(),
                  H5P_DEFAULT) == 0) {
      exist = false;
    }
