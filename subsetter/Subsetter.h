...
public:
  Subsetter(SubsetDataLayers *subsetDataLayers, std::vector<geobox> *geoboxes,
            Temporal *temporal, GeoPolygon *geoPolygon, Configuration *config,
  ...
  int subset(std::string infilename, std::string outfilename,
    ...
    H5Pset_libver_bounds(fileAccessPropList, H5F_LIBVER_LATEST,
                         H5F_LIBVER_LATEST);
    H5::FileAccPropList fileAccessPropListObj(fileAccessPropList);
    this->outfile = H5::H5File(outfilename, H5F_ACC_TRUNC,
                               infile.getCreatePlist(), fileAccessPropListObj);
    ...
    if (!matchingDataFound) {
      ...
      if (remove(outfilename.c_str()))
        LOG_ERROR("Subsetter::subset(): No matching data for the constraints "
                  "specified.  Unable to remove: "
                  << outfilename);
      else
        LOG_INFO("Subsetter::subset(): No matching data for the constraints "
  ...
  virtual std::string retrieveShortName(const H5::H5File &file) {
    ...
    for (std::vector<std::string>::iterator it = shortnamePaths.begin();
         it != shortnamePaths.end(); it++) {
      shortnameFullPath = *it;
      LOG_DEBUG(shortnameFullPath);
      // group
      // "/" is used in the config file to determine whether the short name
      // attribute is under a group or dataset short name attribute path with
      // "/" at the end indicates it is a group path otherwise, it is a dataset
      // path
      if (shortnameFullPath.compare(shortnameFullPath.length() - 1, 1, "/") ==
          0) {
        shortnameFullPath =
        ...
        if (H5Lexists(root.getLocId(), shortnamePath.c_str(), H5P_DEFAULT) >
                0 &&
      ...
      for (int index = 0; index < inputGroup.getNumObjs(); index++) {
        ...
        bool isMetadataGroup =
            (boost::to_upper_copy<std::string>(objectPath)
                 .compare(0, metadataGroup.length(), metadataGroup) == 0)
                ? true
                : false;
        ...
        if (typeName == "group" && !isMetadataGroup &&
          ...
          Coordinate *coor =
              getCoordinate(rootGroup, inputGroup, objectPath, subsetDataLayers,
          ...
          addGroupsRequiringTemporalSubsetting(rootGroup, objectGroup,
                                               objectPath);
        }
      }
    }
   ...
   * the metadata group. Otherwise, the following cases are considered: (1) Only
   * parameter subsetting. (2) Spatial/temporal subsetting, user request only
   * contains unsubsettable datasets. (3) Spatial/temporal subsetting, user
   * request contains subsettable datasets, and output has subsettable datasets.
   *
   * @return true
   * @return false
   */
  bool isMatchingDataFound(H5::H5File infileH5, H5::H5File outfileH5) {
    ...
    // Use a new SubsetDataLayer object to get a list of the datasets/group in
    // the output.
...
protected:
   ...
   */
  void copyAttributes(const H5::H5Object &inputFile, H5::H5Object &outputFile,
    ...
    for (int k = 0; k < inputFile.getNumAttrs(); k++) {
      ...
      if (attribute.getDataType().getClass() == H5T_COMPOUND) {
        if (attribute.getName() != "REFERENCE_LIST") {
          LOG_DEBUG("Subsetter::copyAttributes(): Could not handle attribute "
  ...
  virtual void writeDataset(const std::string &objname,
                            const H5::DataSet &indataset, H5::Group &outgroup,
                            const std::string &groupname,
    ...
    // If the output dimensions of the dataset are unchanged, select the entire
    // input dataspace.
  ...
  virtual Coordinate *getCoordinate(H5::Group &root, H5::Group &ingroup,
                                    const std::string &groupname,
                                    ...
                                    Configuration *config,
                                    bool repair = false) {
    Coordinate *coor = Coordinate::getCoordinate(
        root, ingroup, groupname, shortName, subsetDataLayers, geoboxes,
        temporal, geoPolygon, config);
    return coor;
  }
...
private:
   ...
   */
  int copyH5(H5::Group &in, H5::Group &inRootGroup, H5::Group &out,
             std::string groupname) {
    ...
    if (!isMetadataGroup && config->isGroupSubsettable(shortName, groupname)) {
      ...
      if (geoboxes != NULL || temporal != NULL || geoPolygon != NULL) {
        Coordinate *coor =
            getCoordinate(inRootGroup, in, groupname, subsetDataLayers,
                          geoboxes, temporal, geoPolygon, config);
    ...
    for (int i = 0; i < in.getNumObjs(); i++) {
      ...
      config->addShortNameGroupDatasetFromGranuleFile(shortName,
                                                      objectFullName + "/");
      ...
      if (type_name == "group" &&
        ...
        if (outputFormat == "GeoTIFF" && !isMetadataGroup) {
          requiredDatasets = config->getRequiredDatasetsByFormat(
              outputFormat, groupname + objname + "/", shortName, resolution);
        }
        ...
        if (outgroup.getNumObjs() == 0 && ingroup.getNumObjs() != 0)
          out.unlink(objname);
                 ...
                 (subsetDataLayers->is_dataset_included(groupname + objname) ||
                  std::find(requiredDatasets.begin(), requiredDatasets.end(),
        ...
        if (datasetlinks->isHardLink(objname)) {
          ...
          if (!source.empty() &&
              subsetDataLayers->is_dataset_included(groupname + source) &&
        ...
        if (config->isPhotonDataset(this->getShortName(),
                                    groupname + objname)) {
          LOG_DEBUG("Subsetter::copyH5(): groupname+objname: " << groupname +
                                                                      objname);
          Coordinate *coor = getCoordinate(inRootGroup, in, groupname + objname,
                                           subsetDataLayers, geoboxes, temporal,
                                           geoPolygon, config);
   ...
   * @param indexes The subset dataset indexes.
   */
  void updateTimeRange(const std::string &objname, const H5::DataSet &indataset,
                       IndexSelection *indexes) {
    ...
    config->getMatchingCoordinateDatasetNames(this->getShortName(), datasetName,
                                              timeName, latitudeName,
                                              longitudeName, ignoreName);
    if (!timeName.empty()) {
      ...
      LOG_DEBUG("Subbsetter::updateTimeRange(): Current time range: ("
                << std::fixed << timeRange.first << ", " << timeRange.second
                << ")");
    }
  }
  ...
  bool isCoordinatesAttributeValid(H5::Attribute &attr,
    ...
    // /Soil_Moisture_Retrieval_Data/longitude note: This code doesn't handle
    // GLAS dataset `coordinates` attribute which doesn't provide
    ...
    //       DS_UTCTime_40, but dataset /Data_40HZ/DS_UTCTime_40 is under parent
    //       group
    //
   ...
   */
  bool
  containsSubsettableDatasets(std::vector<std::set<std::string>> datasets) {
    std::string metadataGroup = "/METADATA/";
    ...
    while (it != datasets.end() && !subsettable) {
      ...
      while (set_iter != it->end()) {
        ...
        if (!isMetadataGroup &&
            ...
            std::find(dimScales->begin(), dimScales->end(),
                      datasetName.substr(0, datasetName.size() - 1)) ==
                dimScales->end() &&
            std::find(inconsistentDatasets.begin(), inconsistentDatasets.end(),
                      datasetName.substr(0, datasetName.size() - 1)) ==
                inconsistentDatasets.end()) {
          LOG_DEBUG("Subsetter::containsSubsettableDatasets(): subsettable "
                    << datasetName);
          subsettable = true;
          break;
        }
