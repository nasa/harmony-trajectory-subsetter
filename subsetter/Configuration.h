...
public:
  ...
  getSuperCoordinates(const std::string &shortname) {
    std::map<std::string, std::vector<std::string>> datasets;
  ...
  void getSuperCoordinateGroupname(const std::string &shortname,
    ...
    for (std::map<std::string,
      ...
      if (regex_match(shortname, boost::regex(it->first))) {
        ...
        paths.clear();
        for (std::map<std::string, std::vector<std::string>>::iterator it2 =
                 it->second.begin();
             it2 != it->second.end(); it2++) {
          for (std::vector<std::string>::iterator it3 = it2->second.begin();
   ...
   */
  void getRequiredDatasetsAndResolution(
      const std::string &format, const std::string &groupname,
      const std::string &shortName, std::string &rowName, std::string &colName,
      std::string &latName, std::string &lonName, short &resolution) {
    std::vector<std::string> datasets =
        getRequiredDatasetsByFormat(format, groupname, shortName, resolution);
  ...
  getRequiredDatasetsByFormat(const std::string &format,
                              const std::string &groupname,
                              const std::string &shortname, short &resolution) {
    std::vector<std::string> datasets;
    for (std::map<std::string,
                  std::map<std::string, std::vector<std::string>>>::iterator
      ...
      if (regex_match(format, boost::regex(it->first))) {
        for (std::map<std::string, std::vector<std::string>>::iterator map_it =
                 it->second.begin();
   ...
   * @param resolution - pixel resolution
   */
  void getResolution(const std::string &groupname, const std::string shortname,
                     short &resolution) {
  ...
  std::string getProjection(const std::string &groupname) {
    ...
    for (std::map<std::string, std::string>::iterator it = projections.begin();
  ...
  bool isGroupSubsettable(const std::string &shortName,
    ...
    // if the shortName is configured and group is not specified, the group is
    // not subsettable
  ...
  std::string getTimeCoordinateName(std::string &datasetName,
    ...
    this->getMatchingCoordinateDatasetNames(shortName, datasetNames, timeName,
  ...
  std::string getPhotonGroupOfSegmentGroup(const std::string &shortName,
    ...
    if (isSegmentGroup(shortName, group)) {
      for (std::map<std::string, std::map<std::string, std::string>>::iterator
        ...
        // shortname matched
        if (regex_match(shortName, boost::regex(shortNamePattern))) {
          // get the photon and segment group names which are at the end of path
          // pattern
          std::string pathDelim("/");
          std::vector<std::string> paths;
          ...
          // replace photonGroupName with segmentGroupName to get corresponding
          // segment group'
          photonGroup = group;
    ...
    if (isPhotonDataset(shortName, dataset)) {
      for (std::map<std::string, std::map<std::string, std::string>>::iterator
        ...
        if (regex_match(shortName, boost::regex(shortNamePattern))) {
          std::string pathDelim("/");
          std::vector<std::string> paths;
          split(paths, keyinfo.at("PhotonDataset"),
    ...
    if ((isHeightSegmentRateGroup(shortName, group) &&
      ...
      for (std::map<std::string, std::map<std::string, std::string>>::iterator
        ...
        if (regex_match(shortName, boost::regex(shortNamePattern))) {
          ...
          paths.clear();
          boost::replace_last(freeboardBeamSegmentGroup, groundTrackPattern,
                              groundTrack);
        }
      }
    }
    ...
    if (isHeightSegmentRateGroup(shortName, group) &&
      ...
      for (std::map<std::string, std::map<std::string, std::string>>::iterator
        ...
        if (regex_match(shortName, boost::regex(shortNamePattern))) {
          ...
          groundTrackPattern = paths.at(1);
          paths.clear();
          boost::replace_last(beamFreeboardGroup, groundTrackPattern,
                              groundTrack);
        }
      }
    }
    ...
    if (isFreeboardSegmentHeightsGroup(shortName, group) ||
      ...
      for (std::map<std::string, std::map<std::string, std::string>>::iterator
        ...
        if (regex_match(shortName, boost::regex(shortNamePattern))) {
          ...
          groundTrackPattern = paths.at(1);
          paths.clear();
          boost::replace_last(freeboardSegmentGroup, groundTrackPattern,
                              groundTrack);
        }
      }
    }
  ...
  std::string getLeadsGroup(const std::string &shortName,
    ...
    boost::unordered_map<std::string, std::string> groundTrackMap{
        {"gt1", "gt1l"}, {"gt2", "gt1r"}, {"gt3", "gt2l"},
        {"gt4", "gt2r"}, {"gt5", "gt3l"}, {"gt6", "gt3r"}};
    ...
    if (isFreeboardRateGroup(shortName, group) ||
      ...
      for (std::map<std::string, std::map<std::string, std::string>>::iterator
        ...
        if (regex_match(shortName, boost::regex(shortNamePattern))) {
          ...
          groundTrack = paths.at(1);
          paths.clear();
          split(paths, referenceSurfaceSectionGroup,
                boost::is_any_of(pathDelim));
          groundTrackPattern = paths.at(1);
          paths.clear();
          boost::replace_last(referenceSurfaceSectionGroup, groundTrackPattern,
                              groundTrack);
        }
      }
    }
    ...
    if (isLeadsGroup(shortName, groupname)) {
      for (std::map<std::string, std::map<std::string, std::string>>::iterator
        ...
        if (regex_match(shortName, boost::regex(shortNamePattern))) {
          ...
          boost::replace_last(heightsGroupname, groundTrackPattern,
                              groundTrack);
        }
      }
    }
  ...
  void getDatasetNames(const std::string &shortName,
                       const std::string &groupname, std::string &indexBegin,
                       std::string &count) {
    LOG_DEBUG(
  ...
  void getMatchedDatasetNames(const std::string &datasetname,
                              std::string &indexBegin, std::string &count) {
  ...
  std::string getCountDatasetName(const std::string &shortName,
                                  ...
                                  const std::string &datasetName = "") {
    LOG_DEBUG(
        "Configuration::getCountDatasetName(): ENTER groupname: " << groupname);
  ...
  void getSwathSegmentDatasetNames(const std::string &shortName,
    ...
    boost::unordered_map<std::string, std::string> groundTrack{
        {"gt1l", "gt1"}, {"gt1r", "gt2"}, {"gt2l", "gt3"},
        {"gt2r", "gt4"}, {"gt3l", "gt5"}, {"gt3r", "gt6"}};
  ...
  void groundTrackRename(std::string groupname, std::string &indexBegin,
                         std::string &count) {
    boost::unordered_map<std::string, std::string> groundTrack{
        {"gt1l", "gt1"}, {"gt1r", "gt2"}, {"gt2l", "gt3"},
        {"gt2r", "gt4"}, {"gt3l", "gt5"}, {"gt3r", "gt6"}};
  ...
  void getBeamFreeboardIndexBegin(const std::string &shortName,
    ...
    LOG_DEBUG("Configuration::getBeamFreeboardIndexBegin(): ENTER indexBegin: "
              << indexBegin);
  ...
  void getBeamSegmentDatasetNames(const std::string &shortName,
                                  std::string &indexBegin) {
    LOG_DEBUG("Configuration::getBeamSegmentDatasetNames(): ENTER indexBegin: "
              << indexBegin);
  ...
  bool isShortNameGroupDatasetFromGranuleFile(const std::string &shortName,
                                              const std::string &group) {
    LOG_DEBUG(
        "Configuration::isShortNameGroupDatasetFromGranuleFile(): ENTER group: "
        << group);
  ...
  std::string getVersionNumber(const std::string &shortName) {
    LOG_DEBUG(
        "Configuration::getVersionNumber(): ENTER shortName: " << shortName);
    ...
    for (const auto &keyinfo : shortNameGroupDatasetFromGranuleFile) {
      for (const auto &valueinfo : keyinfo.second) {
        if (keyinfo.first == shortName &&
          ...
          LOG_DEBUG("Configuration::getVersionNumber() FOUND version = 005 "
                    "shortName: "
                    << keyinfo.first << " group: " << valueinfo);
          return ATL10v005;
        } else if (keyinfo.first == shortName &&
                   regex_match(valueinfo, boost::regex(atl10v006))) {
          LOG_DEBUG("Configuration::getVersionNumber() FOUND version = 006 "
                    "shortName: "
                    << keyinfo.first << " group: " << valueinfo);
          return ATL10v006;
        }
...
private:
  ...
  // check if a given string matches any regexp on the list, only search if the
  // string has the pattern since the string can have extra information e.g.
  // value: /profile_1/calibration matches the regexpList which has /profile_1
