...
public:
  ...
  ForwardReferenceCoordinates // main constructor for class
      (std::string groupname, std::vector<geobox> *geoboxes, Temporal *temporal,
  ...
  static Coordinate *getCoordinate(H5::Group &root, H5::Group &ingroup,
    ...
    LOG_DEBUG("ForwardReferenceCoordinates::getCoordinate(): ENTER groupname: "
              << groupname);
    ...
    if (config->subsetBySuperGroup(shortName, segGroupname)) {
      coor = SuperGroupCoordinate::getCoordinate(
          root, forCoor->segGroup, shortName, subsetDataLayers, segGroupname,
          geoboxes, temporal, geoPolygon, config);
    } else {
      coor = Coordinate::getCoordinate(root, forCoor->segGroup, segGroupname,
   ...
   */
  void scanFwdNonFill(long segStartIdx, long segEndIdx, long &firstTrajIndex,
                      long &firstNonFillIdx, int64_t indexBegDataset[]) {
   ...
   * @param indexBegDatset  The input indexBeg dataset array.
   */
  void scanBackNonFill(long segEndIdx, long segStartIdx, long &lastTrajIndex,
                       long &lastNonFillIdx, int64_t indexBegDataset[]) {
    // skip over segment-begin (start) fill values
  ...
  void defineOneSegment(long selectedStartIdx, long selectedCount,
                        long &firstTrajIndex, long &trajSegLength,
    ...
    scanFwdNonFill(selectedStartIdx, lastSelectedIdx, firstTrajIndex,
                   firstIdxNonFill, indexBegDataset);
    ...
    scanBackNonFill(lastSelectedIdx, firstIdxNonFill, lastTrajIndex, lastBegIdx,
                    indexBegDataset);
    ...
    scanFwdNonFill(lastBegIdx + 1, maxIndexBegIdx, nextTrajIndex, nextBegIdx,
                   indexBegDataset);
...
private:
  ...
  void segmentedTrajectorySubset(H5::DataSet *indexBegSet) {
    ...
    for (std::map<long, long>::iterator it = segIndexes->segments.begin();
         it != segIndexes->segments.end(); it++) {
      long selectedStart = it->first;
      long selectedCount = it->second;
      long start = 0, length = 0;

      defineOneSegment(selectedStart, selectedCount, start, length, idxBegSize,
    ...
    if (segIndexes->segments.empty()) {
      ...
      // Note: index-selection start is true to datasets, zero based indexing,
      // whereas start index pulled from indexBegin datasets is one based
      // indexing
      if (start > 0) {
        indexes->addSegment(start - 1, length);
      }
