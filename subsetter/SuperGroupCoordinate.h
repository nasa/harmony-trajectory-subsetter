...
public:
  SuperGroupCoordinate(std::string groupname, std::vector<geobox> *geoboxes,
   ...
   * when multiple sets of lat/lon coordinates references exist within one group
   * and these coordinate references all referencing one delta_time datasets,
   * the datasets under that group will be subsetted using the "super group"
   * rule.
   ...
   * <lat_1,lon_1> or <lat_2,lon_2> or <lat_3,lon_3> and delta_time(if temporal
   *         constraint exists)
   ...
   */
  static Coordinate *getCoordinate(H5::Group &root, H5::Group &ingroup,
    ...
    for (std::map<std::string, std::vector<std::string>>::iterator it =
      ...
      while (!it->second.empty()) {
        config->getMatchingCoordinateDatasetNames(shortname, it->second,
                                                  timeName, latitudeName,
                                                  longitudeName, otherName);
        if (!latitudeName.empty()) {
          sgCoor->latitudes.push_back(latitudeName);
          it->second.erase(
              std::remove(it->second.begin(), it->second.end(), latitudeName),
              it->second.end());
        }
        if (!longitudeName.empty()) {
          sgCoor->longitudes.push_back(longitudeName);
          it->second.erase(
              std::remove(it->second.begin(), it->second.end(), longitudeName),
              it->second.end());
        }
        if (!timeName.empty()) {
          it->second.erase(
              std::remove(it->second.begin(), it->second.end(), timeName),
              it->second.end());
          for (std::vector<H5::Group>::iterator it =
                   sgCoor->superGroups.begin();
               it != sgCoor->superGroups.end(); it++) {
            if (!timeName.empty() &&
                H5Lexists(it->getLocId(), timeName.c_str(), H5P_DEFAULT) > 0) {
    ...
    for (int i = 0; i < sgCoor->latitudes.size(); i++) {
      for (std::vector<H5::Group>::iterator it = sgCoor->superGroups.begin();
           it != sgCoor->superGroups.end(); it++) {
        if (H5Lexists(it->getLocId(), sgCoor->latitudes[i].c_str(),
                      H5P_DEFAULT) > 0) {
          data = new H5::DataSet(it->openDataSet(sgCoor->latitudes[i]));
          sgCoor->checkCoorDatasetSize(data, inconsistentCoorDatasets);
          sgCoor->coorDatasets.insert(std::pair<std::string, H5::DataSet *>(
              sgCoor->latitudes[i], data));
        }
      }
      for (std::vector<H5::Group>::iterator it = sgCoor->superGroups.begin();
           it != sgCoor->superGroups.end(); it++) {
        if (H5Lexists(it->getLocId(), sgCoor->longitudes[i].c_str(),
                      H5P_DEFAULT) > 0) {
    ...
    // if the number of latitude datasets and the number of longitude datasets
    // don't agree, or their dataset sizes are not the same, return Index
    // Selection with retstriction, start=0, length=0
  ...
  virtual IndexSelection *getIndexSelection() {
    ...
    if (temporal != NULL && timeSet != NULL) {
      ...
      LOG_DEBUG("SuperGroupCoordinate::getIndexSelection(): "
    ...
    if ((geoboxes != NULL || geoPolygon != NULL) &&
      ...
      for (int i = 0; i < this->latitudes.size(); i++) {
        ...
        readLatLonDatasets(this->coorDatasets.find(this->latitudes[i])->second,
    ...
    if (geoboxes != NULL && this->coorDatasets.size() != 0)
      ...
      LOG_DEBUG("SuperGroupCoordinate::getIndexSelection(): spatial constraint "
    ...
    if (geoPolygon != NULL && this->coorDatasets.size() != 0)
      ...
      LOG_DEBUG("SuperGroupCoordinate::getIndexSelection(): polygon or lat/lon "
...
private:
  ...
  void spatialPolygonSubset() {
    ...
    LOG_DEBUG("SuperGroupCoordinate::spatialPolygonSubset(): geoboxes.size(): "
              << geoboxes->size());

    spatialBboxSubset();
    LOG_DEBUG("SuperGroupCoordinate::spatialPolygonSubset(): indexes->size(): "
              << indexes->size());
