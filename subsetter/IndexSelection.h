...
public:
  ...
  void addSegment(long newStart, long newLength) {
    ...
    // use-case) Note early loop/function exits, once an insertion point for new
    // segment is found.
    for (it = segments.rbegin(); it != segments.rend(); it++) {
      ...
      if (newStart + newLength - 1 >= existingEnd) {
        ...
        // segment; (following, no overlap) a common use-case - add segment to
        // end of list
        if (newStart > (existingEnd + 1)) {
          // Add segment as a new segment at end.
          segments[newStart] = newLength;
          // Note any intervening following segments have already been caught by
          // this loop
          return;
        }
        // Case A2: [ _ { _ ] _ } - new segment extends existing segment
          ...
          // Note any overlapping following segments have already been caught by
          // this loop
          return;
        }
        // Case A3: { _ [ _ ] _ } - new segment starts before existing segment
        // starts (enclosing)
        else {
          segments.erase(--(it.base()));   // erase existing segment
          addSegment(newStart, newLength); // add this one,
          // using recursion to catch any earlier potentially overlapping
          // segments
          return;
        }
      }
      ...
      {
        // Case B1: [ _ { _ } _ ] - new segment fully enclosed in existing
        // segment - new segment starts before start of existing segment
        if (newStart >= existingStart) {
          return; // do nothing; existing segment in list already covers this
                  // case.
        }

        // Case B2: { _ [ _ } _ ] - new segment starts before existing segment
        // ends
  ...
  friend std::ostream &operator<<(std::ostream &out,
    ...
    std::map<long, long>::iterator it;
    out << "[" << " ";
    for (it = selection.segments.begin(); it != selection.segments.end(); it++)
