...
public:
  ...
  Coordinate(std::string groupname, std::vector<geobox> *geoboxes,
  ...
  static Coordinate *getCoordinate(H5::Group &root, H5::Group &ingroup,
    ...
    for (i = 0; i < numOfObjs; i++) {
      ...
      if (typeName == "dataset") {
        ...
        coor->getCoordinateDatasetNames(data, shortname, latitudeName,
                                        longitudeName, timeName, ignoreName,
                                        coorGroupname);
      }
    }
    // match datasets in the group with the coordinate dataset names in the
    // configuration file
    config->getMatchingCoordinateDatasetNames(
        shortname, allDatasets, timeNameInGroup, latitudeNameInGroup,
        longitudeNameInGroup, ignoreNameInGroup);

    // use groupname as coorGroupname(key for lookup map) if the coordinate
    // dataset names are retrieved by walking through the datasets in the group
    ...
    // if lat/lon/time datasets were found in the group and not in coordinates
    // attribute, use lat/lon/time datasets found in the group
    ...
    if (!longitudeName.empty() &&
        H5Lexists(coorGroup.getLocId(), longitudeName.c_str(), H5P_DEFAULT) >
            0) {
    ...
    //    (arraySize used to instantiate IndexSelection is from the datasets in
    //    the group,
    ...
    if ((!latitudeName.empty() &&
        ...
        (!timeName.empty() &&
         H5Lexists(coorGroup.getLocId(), timeName.c_str(), H5P_DEFAULT) <= 0)) {
      LOG_DEBUG(
          "Coordinate::getCoordinate(): coordinate dataset(s) does not exist");
   ...
   */
  void readLatLonDatasets(H5::DataSet *latSet, H5::DataSet *lonSet, double *lat,
                          double *lon) {
  ...
  void updateEpochTime(H5::DataSet *time) {
    ...
    for (int i = 0; i < time->getNumAttrs(); i++) {
      ...
      if (attrName == "units") {
        ...
        if (regex_search(attrValue, match,
                         boost::regex("\\d{4}[-]\\d{2}[-]\\d{2}"))) {
          epoch = match[0] + timeStr;
          // if the epoch is different from the product epoch in configuration
          // file or default epoch, update it
...
private:
  ...
  void getCoordinateDatasetNames(H5::DataSet dataset, std::string shortname,
                                 std::string &latitudeName,
                                 std::string &longitudeName,
                                 std::string &timeName, std::string &ignoreName,
    ...
    for (int i = 0; i < dataset.getNumAttrs(); i++) {
      ...
      if (attrName == "coordinates") {
        ...
        BOOST_FOREACH (attrDataset, datasets) {
          ...
          // else, if it does contain "..", need to convert it to the correct
          // path
          else {
            absPath = groupname + attrDataset;
          ...
          if (std::find(datasetNames.begin(), datasetNames.end(),
                        attrDataset) == datasetNames.end()) {
            LOG_DEBUG("Coordinate::getCoordinateDatasetNames(): adding "
                      << attrDataset);
            datasetNames.push_back(attrDataset);

            // get matching coordinate dataset names with the configuration file
            config->getMatchingCoordinateDatasetNames(
                shortname, datasetNames, timeName, latitudeName, longitudeName,
                ignoreName);
          }
        }
      }
