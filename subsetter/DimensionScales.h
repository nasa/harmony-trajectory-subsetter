...
public:
  DimensionScales() {
    ...
    datasetScales =
        new std::map<std::string, std::vector<std::pair<std::string, int>> *>();
  }
   ...
   */
  static herr_t dimensionScaleCallback(hid_t did, unsigned idx, hid_t dsid,
                                       void *opdata) {
  ...
  void trackDimensionScales(const H5::DataSet &dataset) {
    ...
    } else // dataset scales
    {
      ...
      // Iterates the callback function through the scales attached to each
      // dimension
      for (int i = 0; i < dimnum; i++) {
        ...
        for (int j = 0; j < numscales; j++) {
          ...
          H5DSiterate_scales(did, i, &j,
                             DimensionScales::dimensionScaleCallback, scale);
          if (!scale->empty()) {
            ...
            if (datasetScales->find(datasetName) == datasetScales->end()) {
              std::vector<std::pair<std::string, int>> *scaleDims =
        ...
        if (numscales != numValidScales)
          LOG_DEBUG("DimensionScales::trackDimensionScales(): WARNING dataset "
                    << datasetName << " has " << numscales - numValidScales
                    << " invalid scale references in the dimension " << dimnum);
      }
    }
  }
