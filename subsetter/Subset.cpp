...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    Subsetter *getMission = new Subsetter(subsetDataLayers, geoboxes, temporal,
                                          geoPolygon, config, outputFormat);
    ...
    if (mission == "ICESAT") {
      subsetter = new IcesatSubsetter(subsetDataLayers, geoboxes, temporal,
                                      geoPolygon, config);
    } else if (mission == "GEDI") {
      subsetter = new SuperGroupSubsetter(subsetDataLayers, geoboxes, temporal,
                                          geoPolygon, config);
    } else // Use the base Subsetter if the mission isn't GEDI or ICESAT.
    {
      subsetter = new Subsetter(subsetDataLayers, geoboxes, temporal,
                                geoPolygon, config, outputFormat);
    }
