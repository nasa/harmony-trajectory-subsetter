...
static const TIFFFieldInfo xtiffFieldInfo[] = {{TIFFTAG_UNITS, 1, 1, TIFF_ASCII,
                                                FIELD_CUSTOM, 0, 0,
...
public:
  void convert(short *row_data, short *col_data, float *lat_dat,
               float *lon_data) {
  ...
  geotiff_converter(std::string outfilename, std::string shortName,
                    H5::Group &outgroup, SubsetDataLayers *subsetDataLayers,
    ...
    for (std::vector<std::set<std::string>>::iterator set_it = datasets.begin();
         set_it != datasets.end(); set_it++) {
      for (std::set<std::string>::iterator it = set_it->begin();
        ...
        if (subsetDataLayers->is_dataset_included(datasetName)) {
          ...
          std::replace(fileDatasetName.begin(), fileDatasetName.end(), '/',
                       '.');
          LOG_DEBUG("geotiff_converter::geotiff_converter(): tifFileName: "
                    << tifFileName);
          LOG_DEBUG("geotiff_converter::geotiff_converter(): fileDatasetName: "
                    << fileDatasetName);
          tifFileName += fileDatasetName + "tif";

          // ToDo: need to refactor this to get lat/lon/row/col and resolution
          // once for the whole group
          //       instead of getting it for each dataset
          config->getRequiredDatasetsAndResolution(
              outputFormat, groupname, shortName, rowName, colName, latName,
              lonName, resolution);
          ...
          convert_to_geotiff(tifFileName.c_str(), "not used", row, col, ds,
                             outputFormat.c_str(), projection.c_str(),
                             resolution, crop, &lat, &lon);
        }
      }
    }
  }

  void convert_to_geotiff(const char *outfilename, const char *ds_name,
                          H5::DataSet &row, H5::DataSet &col, H5::DataSet &ds,
                          const char *outputformat, const char *projection,
                          short resolution = 36, bool crop = false,
                          H5::DataSet *lat = 0, H5::DataSet *lon = 0) {
    ...
    if (get_data_type(ds) == 0 || get_size(row) == 0) {
      LOG_DEBUG("geotiff_converter::convert_to_geotiff(): Skipping "
                << outfilename
    ...
    for (int i = 0; i < numfiles; i++) {
      ...
      LOG_DEBUG(
          "geotiff_converter::convert_to_geotiff(): processing : " << filename);
    ...
    if (strcmp(projection, "GEO") == 0) {
      ...
      std::map<short, double>
          lat_resolution_ratio; // number of rows in geographic projection vs.
                                // EASE-Grid2
      ...
      // left-center point for x. Corner point extents = faction of grid-extent
      // based on row-min and col-min, normalized to 500 row/col size for 36K
      // resolution, scaled by actual resolution, and relative to grid center
      // point. Using EASE-2 Standard Polar Grid - has constant corner-point
      // value.
      laea = true;
    ...
    for (int i = 0; i < tifs.size(); i++) {
      set_tiff_keys(tifs[i], data_type_size, get_data_type(ds), num_rows,
                    num_cols, geotiepoints, pixelscale, units);
    ...
    for (int i = 0; i < num_rows; i++) {
      for (int l = 0; l < numfiles; l++) {
        for (int j = 0; j < num_cols; j++) {
          if (laea &&
              // normalize to 500 row/column grid size, row/column starts from
              // 0-499
              sqrt(pow((float)i / 36.0 * resolution - 249.5, 2) +
                   pow((float)j / 36.0 * resolution - 249.5, 2)) > 250) {
            memcpy((char *)output_line + j * data_type_size,
                   off_earth_fill_value, data_type_size);
          } else if (array_index_map[i][j] == coordinate_size)
            memcpy((char *)output_line + j * data_type_size, fill_value,
                   data_type_size);
          else {
            void *val = ((char *)dataset_data +
                         array_index_map[i][j] * data_type_size * tifs.size() +
                         l * data_type_size);
            memcpy((char *)output_line + j * data_type_size, val,
                   data_type_size);
          }
        }
  ...
  void *get_off_earth_fill_value(H5::DataSet &ds, const char *outputformat) {
    ...
    if (ds.attrExists("_FillValue")) {
      ...
      LOG_DEBUG("geotiff_converter::get_off_earth_fill_value(): Fill Value not "
  ...
  std::string get_units(H5::DataSet &ds) {
    ...
    if (ds.attrExists("units")) {
      ...
      attr.read(attr.getDataType(), buf);
      LOG_DEBUG(
          "geotiff_converter::get_units(): Attribute units exists:" << buf);
      return buf;
    } else
      return std::string();
  }

  void set_tiff_keys(TIFF *tif, size_t data_type_size, int data_type,
                     int num_rows, int num_cols, double *geotiepoints,
                     double *pixelscale, std::string units) {
  ...
  void set_geotiff_keys(GTIF *gtif, const char *projection) {
    ...
    if (!strcmp(projection, "CEA")) {
      ...
      if (!strcmp(projection, "NLAEA")) {
        GTIFKeySet(
            gtif, GTCitationGeoKey, TYPE_ASCII, 0,
            "Lambert Azimuthal Equal Area - North Polar Projection - (WGS84)");
        ...
        GTIFKeySet(
            gtif, GTCitationGeoKey, TYPE_ASCII, 0,
            "Lambert Azimuthal Equal Area - South Polar Projection - (WGS84)");
        GTIFKeySet(gtif, ProjCenterLatGeoKey, TYPE_DOUBLE, 1, -90.0);
        GTIFKeySet(gtif, ProjNatOriginLongGeoKey, TYPE_DOUBLE, 1, 0.0);
        GTIFKeySet(gtif, ProjFalseOriginLongGeoKey, TYPE_DOUBLE, 1, 0.0);
      }
