...
public:
   ...
   */
  static herr_t linkCallback(hid_t groupId, const char *name,
    ...
    if (linfo->type == 0) // hardlink
    {
      ...
      } else // the address has a dataset, the current dataset can then point to
             // that dataset
      {
  ...
  void trackDatasetLinks(const H5::Group &group) {
    ...
    H5Literate(group.getLocId(), H5_INDEX_NAME, H5_ITER_INC, NULL,
               DatasetLinks::linkCallback, (void *)this);
  }
